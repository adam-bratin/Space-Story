    ...
    function OnInspectorGUI () {
		...
		if (foldout2) {
	    	...
	    	if (target.format3D == 0 || target.format3D == 2) {	// side by side
				...
				target.interlaceRows = EditorGUILayout.IntSlider (GUIContent("Rows","Vertical resolution for interlace format"),target.interlaceRows, 1, 2160);		// james increased for 4k display
	    	} else if (target.format3D == 4) { // checkerboard
				target.checkerboardColumns = EditorGUILayout.IntSlider (GUIContent("Columns", "Horizontal resolution for checkerboard format"),target.checkerboardColumns, 1, 3840);	// james increased for 4k display
				target.checkerboardRows = EditorGUILayout.IntSlider (GUIContent("Rows","Vertical resolution for checkerboard format"),target.checkerboardRows, 1, 2160);	// james increased for 4k display
